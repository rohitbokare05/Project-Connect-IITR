...
const ProjectCard = ({ project }) => {
  const handleContactFaculty = () => {
    ...
    const mailtoLink = `mailto:${project.facultyEmail}?subject=${subject}&body=${body}`;
    
    // Try to open email client
    window.open(mailtoLink, '_self');
    
    // Check if it failed (user still on page after 2 seconds)
    setTimeout(() => {
      if (document.hasFocus()) {
        alert(`Please contact the faculty at:\n\n${project.facultyEmail}`);
      }
    }, 2000);
  };
      ...
      <div className="mb-4 space-y-2">
        ...
        <div className="flex items-center text-gray-600">
          ...
          <button
            onClick={(e) => {
              e.stopPropagation();
              handleContactFaculty();
            }}
            className="text-sm text-indigo-600 hover:underline text-left bg-transparent border-none cursor-pointer p-0"
          >
            {project.facultyEmail}
          </button>
        </div>
